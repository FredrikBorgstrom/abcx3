...
mixin ModelRequestMixin<T> on ModelCreator<T> {
  ...
  Stream<U> _get$<U>({
    ...
    final cachedStream = MemCachedStream(
      param: param,
      endpoint: endpoint,
      body: body,
    );
    final existingCachedStream = _cachedStreams.getByRequest(
      cachedStream.serializedRequest,
    );
    ...
    final broadcastStream$$ = FromCallableStream<U>(
      () => authHttp
  ...
  U handleRequestResult<U>(
    Result<U, DioException> result,
    bool nullReturnsEmptyList,
  ) {
    if (result.isSuccess) {
