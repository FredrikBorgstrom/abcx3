...
export class InputGenerator {
    ...
    private getBaseInputClassName() {
        return `${this.model.name}${this.settings.InputSuffix}`;
    }

    private getCreateInputClassName() {
        return `${this.settings.InputCreatePrefix}${this.model.name}${this.settings.InputSuffix}`;
    }

    private getUpdateInputClassName() {
        return `${this.settings.InputUpdatePrefix}${this.model.name}${this.settings.InputSuffix}`;
    }
    ...
    private generateBaseInput() {
        ...
        if (this.settings.InputParentClass) {
            content = content.replace(
                /#{ParentClass}/g,
                `extends ${this.settings.InputParentClass}`,
            );
        }

        if (this.settings.InputParentClassPath) {
            this.addDecoratorToImport(
                new DecoratorHelper(
                    this.settings.InputParentClass + '',
                    this.settings.InputParentClassPath + '',
                ),
            );
        }
    ...
    generateFieldContent(field: DMMF.Field) {
        ...
        const typeMap = PrismaHelper.getInstance().getMapTypeFromDMMF(
            field,
            this.settings.InputValidatorPackage,
        );
        ...
        if (this.settings.GenerateInputSwagger) {
