export const ServiceStubNeverThrow = `
...
export class #{CrudServiceClassName} {
    ...
    async getAll(): Promise<Result<#{Model}[], Error>> {
        try {
            const result = await this.prismaService.#{moDel}.findMany();
            return ok(result);
        } catch (e) {
            return err(new InternalServerErrorException(\`Could not get #{Model} item\`));
        }
    }
    ...
    async create(data: Prisma.#{Model}CreateInput): Promise<Result<#{Model}, Error>> {
        try {
            ...
            return err(new InternalServerErrorException(\`Could not create #{Model} item\`));
        }
    }
    ...
    async getFiltered(
        ...
        try {
            const items = await this.prismaService.#{moDel}.findMany(filter);
            return ok(items);
        }
        catch(e) {
            return err(new InternalServerErrorException('Could not get filtered #{Model} items'));
        }
    }
    ...
    async getFilteredPage(
        ...
        try {
            ...
            return err(new InternalServerErrorException('Could not get #{Model} items.'));
        }
    }
...
async get#{RelationFieldNameCapitalized}(where: Prisma.#{Model}WhereUniqueInput): Promise<Result<#{RelationMethodReturnType}, Error>> {
    try {
        ...
        return err(new InternalServerErrorException(
            \`Could not get #{RelationFieldName} for #{Model}\`,
        ));
    }
}
`;
