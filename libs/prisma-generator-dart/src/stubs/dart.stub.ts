...
export const dartFromJsonEnumArg = `#{PropName}: #{Type}.fromJson(json['#{PropName}'])`;
export const dartFromJsonEnumListArg = `#{PropName}: (json['#{PropName}']).map((item) => #{Type}.fromJson(item)).toList())`;
