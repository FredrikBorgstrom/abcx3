...
class MainGenerator {
    ...
    async generateFiles(options = this.options, settings = this.settings) {
        ...
        let parsedDmmf: any = null;
        if (typeof (options as any)?.datamodel === 'string') {
                console.log('DEBUG: options.datamodel is a string, length:', (options as any).datamodel.length);
                console.log('DEBUG: options.schemaPath:', options.schemaPath || (options as any).schemaPath);
            try {
                let schemaString = (options as any).datamodel;
                
                // Prisma 7: If schemaPath points to a directory, read all .prisma files
                let schemaPath = options.schemaPath || (options as any).schemaPath;
                ...
                if (schemaPath && fs.existsSync(schemaPath)) {
                    ...
                    if (stat.isDirectory()) {
                        ...
                        schemaString = files
                            .map((f: string) => fs.readFileSync(path.join(schemaPath, f), 'utf8'))
                            .join('\n\n');
                        ...
                        schemaString = fs.readFileSync(schemaPath, 'utf8');
                                ...
                                parsedDmmf = await getDMMF({ datamodel: schemaString });
                // Store in options for PrismaHelper access
                (options as any).parsedDmmf = parsedDmmf;
    ...
    private getModels(options: GeneratorOptions, parsedDmmf?: any): DMMF.Model[] {
        // Use parsed DMMF if available (Prisma 7)
    ...
    private getEnums(options: GeneratorOptions, parsedDmmf?: any): DMMF.DatamodelEnum[] {
        // Use parsed DMMF if available (Prisma 7)
